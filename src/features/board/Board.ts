...
function Board() {
  ...
  const allIds = useMemo(() => visibleClients.map(c => c.id as string), [visibleClients]);
  ...
  const selectedRowsProvider = () => visibleClients.filter(c => selectedSet.has(c.id));
      ...
      <div className="min-w-[1480px] border rounded-lg overflow-hidden">
        ...
        <div ref={containerRef} onScroll={onScroll} style={{ maxHeight: viewportHeight, overflowY: 'auto' }}>
          ...
          <div className="divide-y">
            {visibleClients.slice(startIndex, endIndex).map((c, idx) => {
